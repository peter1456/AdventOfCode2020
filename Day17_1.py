...
CYCLES = 6
NEIGHBOURHOOD_3D = np.ones((3, 3, 3), dtype=np.short)
NEIGHBOURHOOD_3D[1, 1, 1] = 0  # center is the actual element


def add_neighbourhood_matrix(neighbourhood_matrix, coor):
    neighbourhood_matrix[coor[0] - 1:coor[0] + 2, coor[1] -
                         1:coor[1] + 2, coor[2] - 1:coor[2] + 2] += NEIGHBOURHOOD_3D
...
initial_state = np.reshape(input_grid, (input_grid.shape[0], input_grid.shape[1], 1))
